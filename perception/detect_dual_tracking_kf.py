"""
Dual detection + tracking using the vanilla (constant-velocity) Kalman filter
instead of the behavioral EKF. Same pipeline as detect_dual_tracking.py but
uses deep_sort.tracker_vanilla_kf.Tracker for standard KF motion model only.
"""
import argparse
import os
import platform
...
SCRIPT_ROOT = FILE.parents[0]
REPO_ROOT = SCRIPT_ROOT
YOLO_ROOT = REPO_ROOT / "yolov9"
DEEP_SORT_ROOT = REPO_ROOT / "deep_sort"
...
from deep_sort.tracker_vanilla_kf import Tracker
...
def load_gt_trajectories(gt_path):
    ...
    """
    gt_trajectories = {}  # id -> list of (frame, cx, cy)
    path = Path(gt_path)
    if not path.exists():
        return None
    with open(path) as f:
...
def run(weights=ROOT / 'yolo.pt', save_plot_name="yash", source=ROOT / 'data/images', data=ROOT / 'data/coco.yaml',
    ...
    # Load YOLO model
    device_str = (device or '').strip().lower()
    if device_str in ('cuda:0', 'cuda', '0') and torch.cuda.is_available():
        ...
        try:
            ...
            if device_str and device_str != 'cpu' and torch.cuda.is_available():
                ...
                print("CUDA not available in PyTorch, using CPU.")
    ...
    # Tracker with vanilla Kalman filter (constant velocity only, no behavioral EKF)
    ...
    if gt:
        ...
        if gt_trajectories:
            print(f"[INFO] Loaded GT trajectories for {len(gt_trajectories)} IDs")
    save_trajectories(
        full_trajectories,
...
def parse_opt():
    parser = argparse.ArgumentParser(description='Detection + tracking with vanilla Kalman filter')
